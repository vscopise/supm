...
function pxe_custom_front_page_sections() {
	remove_all_actions( 'hestia_do_shop_section' );
	add_action( 'hestia_do_shop_section', 'pxe_hestia_front_autoridades' );
 }
 add_action( 'init', 'pxe_custom_front_page_sections' );
 
 function pxe_hestia_front_autoridades() { 
	ob_start();
    include('templates/template-front-autoridades.php');
    echo ob_get_clean();
 }
